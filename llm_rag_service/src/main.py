...
QUESTIONS_CLASSIFICATION_FILE = "question_classification.csv"

ENV_DATA_DIR_PARH = "DATA_DIR_PATH"
DB_CACHE_DIR = "db"
...
class DataProcessor:
    ...
    def load_datasets(self):  # Todo: Ref [Magic]
        try:
            ...
            contacts_path = os.path.join(self.data_dir, CONTACTS_FILE)
            ...
            questions_path = os.path.join(self.data_dir, QUESTIONS_FILE)
            ...
            logger.debug(f"Contacts DataFrame columns: {list(self.contacts_df.columns)}")
            logger.debug(f"Questions DataFrame columns: {list(self.questions_df.columns)}")
...
class MultiModalRetriever:
    ...
    def _create_vectorstore(self):  # Only contacts using
        ...
        return Chroma.from_documents(
            ...
            persist_directory=os.path.join(self.data_dir, DB_CACHE_DIR)
        )
...
class CityAssistant:
    def __init__(self, data_path: str) -> None:
        ...
        self.__init_classification_model(data_path)

    def __init_classification_model(self, data_path: str) -> None:
        try:
            data = pd.read_csv(os.path.join(data_path, QUESTIONS_CLASSIFICATION_FILE))

            # Разделяем данные на признаки (X) и метки (y)
            x, y = data['question'], data['label']  # TODO: Ref [Magic]
            ...
            # Оценка модели
            logger.info(f'Classification model Accuracy: {accuracy_score(y_test, y_pred)}')
    ...
    def process_query(self, query: str) -> QueryResponse:
        try:
            ...
            logger.info(f'Предсказанный ответ: {prediction[0]}. Запрос: {query}')
...
# Assistant
data_dir_path = os.getenv(ENV_DATA_DIR_PARH)
if not data_dir_path:
    raise MissingEnvironmentVariableError(ENV_DATA_DIR_PARH)
